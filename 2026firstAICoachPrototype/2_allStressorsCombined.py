"""
Same as 2_baseline.py but for the allVariablesCombined setup:
every body region uses the same combined stressors (knee + face + arm).
...
def _candidates_config_all_variables_combined() -> dict:
    """Candidates config where every body region uses the same combined list (knee + face + arm stressors)."""
    base = get_candidate_config("new")
    knee = base["kneePain"]["physicalLoad"]
    face = base["facePain"]["physicalLoad"]
    arm = base["armPain"]["physicalLoad"]
    combined = list(np.unique(knee + face + arm))
    combined = [str(v) for v in combined]
    config = copy.deepcopy(base)
    for pain_type in config:
        config[pain_type] = {"physicalLoad": combined}
    return config


RUN_NAME = "allStressorsCombined"
DATA_VERSION = "new"
USE_DEBUG_GRID = True
...
def main() -> None:
    ...
    candidates_config = _candidates_config_all_variables_combined()
    print("All body regions use the same combined stressors (allVariablesCombined).")

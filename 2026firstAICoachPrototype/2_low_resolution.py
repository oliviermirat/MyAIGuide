"""
Same as 2_baseline.py but for the Low Resolution stressors setup:
excludes some stressors, discretizes others to 0/1/2, binarizes climbingMaxEffortIntensity.
...
STRESSORS_TO_EXCLUDE = [
    "phoneTime",
    "timeSpentRidingCar",
]

LOW_RESOLUTION_COLUMNS = [
    "surfCumBpmAbove110",
...
def _discretize_low_resolution_columns(
    df_train_and_val,
    df_train,
    df_val,
    df_test,
) -> None:
    for col in LOW_RESOLUTION_COLUMNS:
        ...
        q20, q60 = np.nanpercentile(full, [20, 60])

        def bin_values(df):
            if col not in df.columns:
                return
            x = df[col].values
            df[col] = np.where(x < q20, 0, np.where(x <= q60, 1, 2))

        bin_values(df_train_and_val)
        bin_values(df_train)
        bin_values(df_val)
        bin_values(df_test)


def _binarize_climbing_max_effort(
    df_train_and_val,
    df_train,
    df_val,
    df_test,
) -> None:
    col = "climbingMaxEffortIntensity"
    for df in (df_train_and_val, df_train, df_val, df_test):
        if col not in df.columns:
            continue
        x = df[col].values
        df[col] = np.where((x == 0) | (x == 1), 0, 1)


def _filter_candidates_config(config: dict) -> dict:
    filtered = copy.deepcopy(config)
    for pain_type in filtered:
        for group in ("physicalLoad", "allVariables"):
            if group in filtered[pain_type]:
                filtered[pain_type][group] = [
                    v for v in filtered[pain_type][group]
                    if v not in STRESSORS_TO_EXCLUDE
                ]
    return filtered


RUN_NAME = "lowResolution"
DATA_VERSION = "new"
USE_DEBUG_GRID = True
...
def main() -> None:
    ...
    print("Excluding stressors:", STRESSORS_TO_EXCLUDE)
    ...
    candidates_config = _filter_candidates_config(get_candidate_config(DATA_VERSION))
    ...
    _discretize_low_resolution_columns(dfTrainAndVal, dfTrain, dfVal, dfTest)
    _binarize_climbing_max_effort(dfTrainAndVal, dfTrain, dfVal, dfTest)

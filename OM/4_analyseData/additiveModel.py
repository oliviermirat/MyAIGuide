...
def additiveModel(sympNum,enviInfo,labels,symptoms,environment,xaxis,averagingwindow,show=0,relativeScaler=0):
	plt.subplot(3,1,1)
	if (relativeScaler):
	...
	for idx,info in enumerate(enviInfo):
		...
		print(labels[1+idx],'max: ',max(environment[:,numCol]),' ; coeff: ',coeff)
		envir+=coeff*curEnv
	putLegend()

...
environment[:, 0] = (
    environment[:, 0] + environment[:, 34] * 60 - environment[:, 39]
)  # Add Ubuntu time to Computer Time minus screen saver. #
environment[:, 35] = (
...
addMod.additiveModel(1, select, labels, symptoms, environment, xaxis, averagingwindow)

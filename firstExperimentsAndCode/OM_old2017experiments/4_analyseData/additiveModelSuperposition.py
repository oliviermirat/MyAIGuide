...
def additiveModel(sympNum,enviInfo,labels,symptoms,environment,xaxis,averagingwindow,averagingwindow2,averagingwindowBig,sympThres,showw=1,relativeScaler=0):
	...
	for i in range(averagingwindow,len(envir)):
		accumulEnvir[i] = np.average(envir[i-averagingwindow:i]) # + 0.1*envir[i]+0.03*envir[i-1]
	...
	for i in range(averagingwindow2,len(envir)):
		accumulEnvir2[i] = np.average(envir[i-averagingwindow2:i]) # + 0.1*envir[i]+0.03*envir[i-1]


	accumulEnvirBig = np.zeros(len(envir))
	for i in range(0,averagingwindowBig):
		accumulEnvirBig[i] = np.average(envir[0:averagingwindowBig-1]) #- (averagingwindowBig-i)*(0.06/averagingwindowBig) # envir[i]
	...
	for i in range(0,averagingwindowBig):
		moySympt[i] = np.average(symptoms[0:averagingwindowBig-1,sympNum]) #- (averagingwindowBig-i)*(0.06/averagingwindowBig) # envir[i]
	for i in range(averagingwindowBig,len(envir)):
	...
	for i in range(0,len(difference)):
		if (difference[i]<0):
			...
			difference[i] = (difference[i] ) / (accumulEnvirBig[i]/moySympt[i]) #np.exp(0.00001*accumulEnvirBig[i])
	...
	for i in range(0,len(difference2)):
		if (difference2[i]<0):
			...
			difference2[i] = (difference2[i] )/ (accumulEnvirBig[i]/moySympt[i]) #np.exp(0.00001*accumulEnvirBig[i])
	...
	symptomsAverage[0]=symptomsAverage[2]
	symptomsAverage[1]=symptomsAverage[2]
	plt.plot(xaxis,pandas.rolling_mean(minMaxScaler(maxDiff),7),label='Activity Averaged')
	plt.plot(xaxis,pandas.rolling_mean(minMaxScaler(symptomsAverage),7),label='Symptom Averaged')
	putLegend()

...
def addInsultIntensityColumns(data, columnList, window, windowMaxInsultDiff):
  ...
  for var in columnList:
    data[var+'InsultIntensity'] = data[var].rolling(window).mean().shift(periods=1)
    for i in range(0, window):
      data[var+'InsultIntensity'][i] = 0

  for var in columnList:
    data[var+'MaxInsultDiff'] = data[var+'InsultIntensity'].rolling(windowMaxInsultDiff).min().shift(periods=1)
    for i in range(0, windowMaxInsultDiff):
      data[var+'MaxInsultDiff'][i] = 0
    data[var+'MaxInsultDiff'] = data[var+'InsultIntensity'] - data[var+'MaxInsultDiff']
...
def getPainAboveThreshold(data, columnName, painThresh):
  ...
  for i in range(1, len(data)-1):
    if float(data[columnName][i]) >= painThresh and (float(data[columnName][i-1]) >= painThresh or float(data[columnName][i+1]) >= painThresh):
